...
class DCAMERR(IntEnum):
    ...
    DELAYEDFRAME = -2147479799  # 0x80000f09, the frame waiting re-load from hardware buffer with SNAPSHOT(EX) of DEVICEBUFFER MODE
    FAILRELOADFRAME = -2147479798  # 0x80000f0a, failed to re-load frame from hardware buffer with SNAPSHOT(EX) of DEVICEBUFFER MODE
    CANCELRELOADFRAME = -2147479797  # 0x80000f0b, cancel to re-load frame from hardware buffer with SNAPSHOT(EX) of DEVICEBUFFER MODE
...
class DCAM_IDPROP(IntEnum):
    ...
    IMAGEDETECTOR_PIXELWIDTH = 4327440  # 0x00420810, R/O, micro-meter, "IMAGE DETECTOR PIXEL WIDTH"
    IMAGEDETECTOR_PIXELHEIGHT = 4327456  # 0x00420820, R/O, micro-meter, "IMAGE DETECTOR PIXEL HEIGHT"
    IMAGEDETECTOR_PIXELNUMHORZ = 4327472  # 0x00420830, R/O, long,  "IMAGE DETECTOR PIXEL NUM HORZ"
    IMAGEDETECTOR_PIXELNUMVERT = 4327488  # 0x00420840, R/O, long,  "IMAGE DETECTOR PIXEL NUM VERT"
    # frame bundle
    ...
    PRIMARYBUFFER_TOTALBYTES = 16715824  # 0x00FF1030, R/W, long,   "PRIMARY BUFFER TOTALBYTES"
    PRIMARYBUFFER_TOTALBYTES_MB = 16715840  # 0x00FF1040, R/W, long,    "PRIMARY BUFFER TOTALBYTES MB"
